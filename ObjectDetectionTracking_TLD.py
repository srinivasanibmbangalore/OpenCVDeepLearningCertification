...
__status__ = "Week 11 Project 4: Soccer Ball Detection and Tracking:  24th Nov 2020"

'''
Usage: python ObjectDetectionTracking_TLD.py
Video file outputsoccer-ball.mp4,yolo weights,yolo config
and coco dataset classnames 
must be in the same working directory.
Once the file is run, the solution attempts to first detect 
the soccer ball whose class id is 32. if it detects, KCF tracker
is commenced. If tracking fails, detection again commences.
...
frmCntr=0
trackerList=[]
(left1, top1, right1, bottom1) = (0,0,0,0)
green=(0,255,0)
blue=(255,0,0)
red=(0,0,255)
...
while True:
    ...
    if ( len(trackerList) == 0):
        left1,top1,right1,bottom1 = detect(frame,net)
        print("1st Detection: ",left1,",",top1,",",right1,",",bottom1)
        bbox1=(left1,top1,right1,bottom1)
        ok = tracker.init(frame, bbox1)
        trackerList.append(bbox1)
        cv2.putText(frame, "Detected", (20, 80),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.75, blue, 2)
    else:
        ok, bbox2 = tracker.update(frame)
        if ok:
            # Tracking success
            p1 = (int(bbox2[0]), int(bbox2[1]))
            p2 = (int(bbox2[0] + bbox2[2]), int(bbox2[1] + bbox2[3]))
            cv2.rectangle(frame, p1, p2, green, 2, 1)
            cv2.putText(frame, "Tracking", (20, 80),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.75, green, 2)


        else:
            # Tracking failure
            cv2.putText(frame, "Tracking failure Detection", (20, 80),
            ...
            #ok = tracker.init(frame, bbox1)
            #trackerList.append(bbox1)
            #cv2.putText(frame, "Detected", (20, 80),cv2.FONT_HERSHEY_SIMPLEX, 0.75, blue, 2)
